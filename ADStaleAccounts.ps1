...
function AdminAlertEmail($Body){
	if ($Body -notlike "*Could not create SSL/TLS secure channel.*") {
		...
		$From = $env:COMPUTERNAME + "@hulu.com"
		$To = $env:ALERTSENDTO
		$SMTPHost = $env:SMTP
...
function Compose30DayEmail($30DayUserList,$SendToList) {
    ...
    $Body = @"
        <p><h4>Dear Hulugans! <br>Please review the information below.</br></h4></p>
        <p>You may manage one of the following user accounts which are being flagged as inactive.</p>
        <p>The user(s) below should sign into <a href="https://okta.com/">okta.com</a> to prevent being disabled and offboarded. If the user is no longer with Hulu please IMMEDIATELY submit a Service-Now ticket.</p>
...
function Compose55DayEmail($55DayUserList,$SendToList) {
    ...
    <p>The below user(s) should sign into <a href="https://okta.com/">okta.com</a> today to prevent being disabled and offboarded. If the user is no longer with Hulu please IMMEDIATELY submit a Service-Now ticket.</p>
